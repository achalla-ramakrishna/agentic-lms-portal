...
export const ARTIFACT_GLOSSARY: Record<string, string> = {
  ...
  "pr document writer skill": "A packaged skill that assembles a PR's description and evidence pack (what changed, what ran, what passed) from the diff and test output, instead of an author writing it by hand.",
  "allure reports": "A test-report format that renders results as a browsable dashboard (pass/fail history, steps, attachments) instead of a raw log, so a reviewer can inspect a test run without re-running it.",
  "playwright traces": "Playwright's recorded trace of a test run (DOM snapshots, network, console) attached as evidence so a reviewer can see exactly what happened without re-running the test themselves.",
  ...
  "/clear": "A command that ends the current conversation and starts a fresh session with empty context, for a genuinely new task rather than one still billing stale history.",
  "/status": "A command that reports the current session's model, effort level, and active tools/rules — a quick check of what's actually configured before you spend tokens.",
  ...
  "ponytail skill": "A packaged skill that reviews a diff for unrequested scope (extra abstractions, unused flexibility) and trims it, since over-engineered output costs more tokens to generate and review.",
  rtk: "Rust Token Killer — a named tool referenced in this competency's guidebook for compressing noisy command output before it's fed back into an agent's context.",
