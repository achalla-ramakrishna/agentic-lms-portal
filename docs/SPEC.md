...
- [x] **Chunk 1 — Toolchain setup** (this commit): `AGENTS.md`/`CLAUDE.md`,
  guardrail hook + suggested settings, secret-scan pre-commit hook, this
  spec, architecture doc, ADRs, Next.js skeleton app that boots.
- [ ] **Chunk 2 — Content model & seed**: `prisma/schema.prisma`,
  `content/seed.json` generated from the exercise-set repo's READMEs,
  `prisma/seed.ts`, competency/exercise read-only pages (no auth yet).
